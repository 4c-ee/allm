...
impl ProxyAuth {
  ...
  /// or a recognised header presents the configured key. Two forms are
  /// accepted, both compared in constant time:
  ///
  /// - `Authorization: Bearer <key>` — the API path (OpenAI SDKs,
  ///   agent harnesses).
  /// - `Authorization: Basic base64(user:<key>)` — the browser path.
  ///
